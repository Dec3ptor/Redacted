...
function selectedMethod(){
  var el=document.querySelector('input[name="redactMethod"]:checked');
  return el?el.value:'flatten';
}
function useFlattening(why){
  var el=document.querySelector('input[name="redactMethod"][value="flatten"]');
...
async function buildBySurgery(){
  if(!window.BlackoutSurgical)return{ok:false,reason:'The text editor did not load.'};
  var boxes=pages.map(function(p){
    return p.rects.map(function(r){return{x:r.x,y:r.y,w:r.w,h:r.h};});
  });
  var bytes=new Uint8Array(await sourceFile.arrayBuffer());
  ...
  return{ok:true,blob:new Blob([res.bytes],{type:'application/pdf'})};
}
